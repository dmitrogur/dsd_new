...
static int squelch_open = 0;             // Состояние шумодава (0=закрыт, 1=открыт)
static float moving_avg_power = -100.0f; // Начальное значение средней мощнос
...
int getSymbol(dsd_opts *opts, dsd_state *state, int have_sync)
{
  ...
  for (i = 0; i < state->samplesPerSymbol; i++) // right HERE
  {
    ...
    {
      if ((state->lastsynctype >= 10 && state->lastsynctype <= 13) || state->lastsynctype == 32 || state->lastsynctype == 33 || state->lastsynctype == 34 || state->lastsynctype == 30 || state->lastsynctype == 31)
      {
    ...
    else
    {
      if (state->rf_mod == 0)
      {
        ...
        else if ((state->lastsynctype >= 10 && state->lastsynctype <= 13) || state->lastsynctype == 32 || state->lastsynctype == 33)
          l_edge = 1;
        else
          l_edge = 2; // P25 and NXDN96 perform better, DMR doesn't seem to care too much, but may favor 1 and not 2
